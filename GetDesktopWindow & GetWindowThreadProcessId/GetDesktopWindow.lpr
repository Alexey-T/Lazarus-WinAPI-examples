...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - GetDesktopWindow example'}

{$R GetDesktopWindow.Res}

program _GetDesktopWindow;
  { Win32 API function - GetDesktopWindow and GetWindowThreadProcessId example}
...
const
  AppNameBase  = 'GetDesktopWindow and GetWindowThreadProcessId Example';
...
const
  GetDesktopWindow_Call         = 'GetDesktopWindow : HWND;';
  GetWindowThreadProcessId_Call = 'GetWindowThreadProcessId(Window : HWND; '  +
                                  'ProcessId : PDWORD) : DWORD';


  DesktopWindow      : HWND  = 0;
  DesktopProcessId   : DWORD = 0;
  DesktopThreadId    : DWORD = 0;
...
begin
  ...
  case Msg of
    ...
    begin
      ...
      DesktopThreadId := GetWindowThreadProcessId(DesktopWindow,
                                                 @DesktopProcessId);

      StrFmt(Buf, '%s%x', [HexIndicator, DesktopWindow]);
      StrFmt(Values[IdxHandle],    '%8s', [Buf]);

      StrFmt(Values[IdxProcessId], '%8d', [DesktopProcessId]);


      StrFmt(Buf, '%s%x', [HexIndicator,   DesktopThreadId]);
    ...
    begin
      ...
      TextOut(ps.hdc,
              ...
              ClientRect.Bottom - 2 * TextSize.cy - TextSize.cy div 3,
              Buf,
              lstrlen(Buf));

      { draw the function call for GetWindowThreadProcessId                   }

      lstrcpy(Buf, GetWindowThreadProcessId_Call);
    ...
    begin
      case LOWORD(wParam) of
        IDM_HIGHLIGHT_DESKTOP_WINDOW:
        begin
          for k := 1 to 10 do    { MUST be an EVEN number of times            }
          begin
            DrawWindowFrame(DesktopWindow);
            sleep(100);
          end;
