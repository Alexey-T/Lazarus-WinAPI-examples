...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - DebugBreak example'}

{$R DebugBreak.Res}

program _DebugBreak;
  { Win32 API function - DebugBreak example                                  }

  { NOTE: DebugBreak() is implemented as "int 3; ret" which makes it very    }
  {       easy to implement as a assembler pascal procedure                  }

uses Windows, Messages, Resource, SysUtils;

const
  AppNameBase  = 'DebugBreak()';
