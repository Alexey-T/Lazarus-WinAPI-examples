...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - SetLastError example'}

{$R SetLastError.Res}

program _SetLastError;
  { Win32 API function - SetLastError example                                 }

uses Windows, Messages, Resource, SysUtils;

const
  AppNameBase  = 'SetLastError';
...
const
  SetLastError_Call  = 'SetLastError (ErrorCd : DWORD);';

  USER_BIT           = $20000000;               { bit 29 on                   }
  LOADLIBRARY_STRING = ':A:\AFilename';         { a bad filename              }
                                                { error $A1 - invalid path    }
var
  ps         : TPAINTSTRUCT;
  ClientRect : TRECT;
...
begin
  ...
  case Msg of
    WM_CREATE:
    begin
      { reset the last error code to zero (should already be zero) and        }
      { check the corresponding menu item                                     }

      PostMessage(Wnd, WM_COMMAND, IDM_RESET, 0);
      exit;
    end;
    ...
    begin
      ...
      {-----------------------------------------------------------------------}
      { get the last error code and display it                                }

      StrFmt(Buf, 'Last Error = $%8.8x', [GetLastError]);
    ...
    begin
      case LOWORD(wParam) of
        IDM_USER:
        begin
          { create custom/user error code                                     }

          SetLastError(1 or USER_BIT);
        ...
        IDM_LOADLIBRARY:
        begin
          { do an invalid call to LoadLibrary                                 }

          LoadLibrary(LOADLIBRARY_STRING);
          InvalidateRect(Wnd, nil, TRUE);
