...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - QueryPerformanceCounter example'}

{$R QueryPerformanceCounter.Res}

program _QueryPerformanceCounter;
  { Win32 API function - QueryPerformanceCounter example                      }

uses Windows, Messages, Resource, SysUtils;

const
  AppNameBase  = 'QueryPerformanceCounter';
...
const
  QueryPerformanceCounter_Call
    = 'QueryPerformanceCounter (PerfCnt : TLargeInteger) : BOOL;';

  Supported            : BOOL          = FALSE;

  {$ifdef VER90}
    PerformanceCounter : TLargeInteger = (QuadPart:0);
  {$endif}

  {$ifdef FPC}
    PerformanceCounter : TLargeInteger = 0;
  {$endif}
...
begin
  ...
  case Msg of
    ...
    begin
      ...
      Supported := QueryPerformanceCounter(PerformanceCounter);
    ...
    begin
      ...
      {-----------------------------------------------------------------------}
      { display the value of the performance counter                          }
      ...
      lstrcpy(Buf, 'Performance Counter value : ');
      TextOut(ps.hdc,
              ClientRect.Right  div 2 + MARGIN,
      ...
      {$ifdef VER90}
        StrFmt(Buf, '%.0n', [PerformanceCounter.QuadPart]);
      {$endif}

      {$ifdef FPC}
        StrFmt(Buf, '%.0n', [PerformanceCounter + 0.0]);
      {$endif}
      ...
      lstrcpy(Buf, QueryPerformanceCounter_Call);
