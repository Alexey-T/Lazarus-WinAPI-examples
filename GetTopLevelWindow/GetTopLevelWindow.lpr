...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - GetTopLevelWindow example'}
...
const
  AppNameBase  = 'GetTopLevelWindow Example - UNDOCUMENTED';
...
const
  GetTopLevelWindow_Call = 'GetTopLevelWindow (Wnd : HWND) : HWND;';
  ...
  LINE_CNT             = 3;  { number of lines to be output                   }
  ...
  SAME                 = 'same as framed window handle';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if p = YES then
          begin
            StrFmt(DataArray[3], '%s', [SAME]);
          end
          else
          begin
            StrFmt(DataArray[3], '$%8.8X', [GetTopLevelWindow(FramedWindow)]);
          end;
        end;
      ...
      SetTextAlign(ps.hdc, TA_CENTER or TA_BOTTOM);
      lstrcpy(Buf, GetTopLevelWindow_Call);
