...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - GetWindowTextLength example'}

{$R GetWindowTextLength.Res}

program _GetWindowTextLength;
  { Win32 API function - GetWindowTextLength example                          }
...
const
  AppNameBase  = 'GetWindowTextLength Example';
...
const
  GetWindowTextLength_Call = 'GetWindowTextLength (Wnd : HWND) : integer;';
  ...
  { be the same size as the LabelsArray.  (defining a constant for the array  }

  DataBuf      : packed array[1..LINE_CNT] of packed array[0..1023] of char
    = (#0, #0);
  ...
  {---------------------------------------------------------------------------}
  { this is the value returned by GetWindowTextLength                         }

  WindowTextLen        : integer = 0;
...
begin
  ...
  case Msg of
    ...
    begin
      ...
      begin
        {---------------------------------------------------------------------}
        { show the caption text of the window the mouse is currently on       }

        GetWindowText(FramedWindow, DataArray[1], sizeof(DataBuf[1]));
        WindowTextLen := GetWindowTextLength(FramedWindow);

        StrFmt(DataArray[2], '%d', [WindowTextLen]);
      ...
      SetTextAlign(ps.hdc, TA_CENTER or TA_BOTTOM);
      lstrcpy(Buf, GetWindowTextLength_Call);
