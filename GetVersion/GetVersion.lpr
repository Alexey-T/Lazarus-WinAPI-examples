...
{$LONGSTRINGS    OFF}
{$WRITEABLECONST ON}
{$DESCRIPTION    'Win32 API function - GetVersion example'}

{$R GetVersion.Res}

program _GetVersion;
  { Win32 API function - GetVersion example                                   }

uses Windows, Messages, Resource, SysUtils;

const
  AppNameBase  = 'GetVersion';
...
begin
  ...
  case Msg of
    ...
    begin
      ...
      Values[1] := Version.Windows.Major;
      Values[2] := Version.Windows.Minor;
      Values[3] := Version.BuildInfo;
    ...
    begin
      ...
      {-----------------------------------------------------------------------}
      { display the result of GetVersion                                      }
      ...
      begin
        TextOut(ps.hdc,
                ...
                ClientRect.Bottom div 2 + (I - 3) * TextSize.cy,
                Labels[I],
                lstrlen(Labels[I]));
      end;
